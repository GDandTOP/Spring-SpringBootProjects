...
public class BoardController {
	...
	public ModelAndView boardWrite(@ModelAttribute Board board,@RequestParam(name="file",required=false) MultipartFile file) {
		ModelAndView mav = new ModelAndView();
		try {
			...
			e.printStackTrace();
			mav.addObject("err","게시판 글 등록이 실패했습니다.");
			mav.setViewName("error");
		}
		return mav;
	}
	...
	public ModelAndView detail(@RequestParam("num") Integer num) {
		...
		{
			e.printStackTrace();
			mav.addObject("err","상세 정보 조회 실패다!");
			mav.setViewName("error");
		}
		return mav;
	}
